...
class _RadioPageState extends State<RadioPage> {
  ...
  void initState() {
    ...
    _audioPlayer.onPlayerStateChanged.listen((PlayerState s) {
      ...
      setState(() {
        _playerState = s;
        if (s == PlayerState.playing || s == PlayerState.stopped ||
            s == PlayerState.paused || s == PlayerState.completed) {
          _isTuning = false;
        }
      });
    });
  ...
  Future<void> _saveFavourites() async {
    await _prefs?.setString(
        'favourite_stations', json.encode(_favouriteStations));
  }
  ...
  Future<void> _cacheStations(String countryCode,
      List<dynamic> stations) async {
    ...
    showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          ...
          content: const Text(
              'Are you sure you want to permanently delete all your favourite station presets?'),
          actions: [
            TextButton(
  ...
  void _toggleFavourite(dynamic station) {
    setState(() {
      final index = _favouriteStations.indexWhere((s) =>
  ...
  void _renamePreset(int index) {
    final TextEditingController controller = TextEditingController(
        text: _favouriteStations[index]['name']);
    showDialog(
      context: context,
      builder: (context) {
        return AlertDialog(
    ...
    showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          ...
          content: Text(
              'Are you sure you want to remove "${_favouriteStations[index]['name']}" from your favourites?'),
          actions: [
            TextButton(
              onPressed: () => Navigator.pop(context),
              child: const Text('Cancel'),
  ...
  Future<void> _getCountries() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        var canada = countries.firstWhere((c) => c['name'] == 'Canada',
            orElse: () => null);
        if (canada != null) {
  ...
  Future<void> _handleCountryChange(String countryCode,
    ...
    if (fetchedStations == null) {
      try {
        final response = await http.get(Uri.parse(
            '$_apiBaseUrl/json/stations/bycountrycodeexact/$countryCode'))
    ...
    final List<Map<String, dynamic>> subdivisions = subdivisionNames.map((
        name) {
      return <String, dynamic>{
        'name': name,
        'stationcount': onlineStations
            .where((s) => s['state'] == name)
            .length,
      };
    }).toList();

    if (subdivisions.length > 1) {
      subdivisions.insert(0, <String, dynamic>{
        'name': 'All',
        'stationcount': onlineStations.length
      });
    }
  ...
  void _playStation(String url, String name, String country,
      {double? needlePos, String? favicon, String? homepage}) {
    if (!_isPowerOn) {
      setState(() {
        _isPowerOn = true;
      });
  ...
  void _playPreset(int index) {
    ...
    if (filteredIdx >= 0) {
      pos = filteredIdx /
          (_filteredStations.length > 1 ? _filteredStations.length - 1 : 1);
    }
            ...
            children: <Widget>[
                ...
                Padding(
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      IconButton(
                        icon: const Icon(
                            Icons.delete_sweep, color: Colors.redAccent),
                        onPressed: _confirmClearFavourites,
                        tooltip: 'Clear All',
                      ),
                    ],
                  ...
                  child: Column(
                    children: [
                      ...
                      RotaryDial(
                        ...
                        onNeedleChanged: (pos) =>
                            setState(() => _needlePosition = pos),
                        backgroundColor: playerBackgroundColor,
                        searchTerm: _selectedSubdivision != null &&
                            _selectedSubdivision != 'All'
                            ? '$_selectedSubdivision, $_selectedCountryName'
                            : _selectedCountryName,
                      ),
                      const SizedBox(height: 20),
                      Text(
                        _isPowerOn ? (_currentlyPlayingStation ??
                            'Select a station') : 'Radio Off',
                        ...
                        Column(
                          children: [
                                ...
                                children: [
                                  ...
                                  Column(
                                    children: [
                                      ...
                                      RotaryKnob(
                                        label: 'PRESETS',
                                        value: _presetKnobIndex /
                                            (_favouriteStations.length > 1
                                                ? _favouriteStations.length - 1
                                                : 1),
                                        onChanged: (val) {
                                          setState(() {
                                            _presetKnobIndex = (val *
                                                (_favouriteStations.length - 1))
                                                .round();
                                          });
                                        },
                                        onTap: () =>
                                            _playPreset(_presetKnobIndex),
                                      ),
                                    ],
                                  ),
                                ],
                            ...
                            ReorderableWrap(
                              ...
                              children: _favouriteStations
                                  .asMap()
                                  .entries
                                  .map((entry) {
                                final int index = entry.key;
                                final dynamic station = entry.value;
                                final isPlaying = _currentlyPlayingStation !=
                                    null &&
                                    _currentlyPlayingStation!.startsWith(
                                        station['name']);
                                final isDialed = index == _presetKnobIndex;
                                ...
                                return Tooltip(
                                  ...
                                  child: GestureDetector(
                                    ...
                                    onTap: _isReorderMode ? null : () {
                                      setState(() => _presetKnobIndex = index);
                                      final filteredIdx = _filteredStations
                                          .indexWhere((s) =>
                                      s['stationuuid'] ==
                                          station['stationuuid']);
                                      double? pos;
                                      if (filteredIdx >= 0) {
                                        pos = filteredIdx /
                                            (_filteredStations.length > 1
                                                ? _filteredStations.length - 1
                                                : 1);
                                      }
                                      _playStation(station['url_resolved'],
                                          station['name'], station['country'],
                                          needlePos: pos,
                                          favicon: station['favicon'],
                                          homepage: station['homepage']);
                                    },
                                    onLongPress: _isReorderMode ? null : () =>
                                        _showPresetOptions(index),
                                    onSecondaryTap: _isReorderMode
                                        ? null
                                        : () => _confirmRemovePreset(index),
                                    child: AnimatedContainer(
                                      ...
                                      width: isDialed ? 55 : 45,
                                      height: isDialed ? 55 : 45,
                                      decoration: BoxDecoration(
                                        ...
                                        boxShadow: [
                                          BoxShadow(
                                            color: Colors.black.withValues(
                                                alpha: isDialed ? 0.4 : 0.2),
                                            blurRadius: isDialed ? 8 : 4,
                                            offset: Offset(0, isDialed ? 4 : 2),
                                          )
                                        ],
                                      ),
                                      child: Center(
                      ...
                      ElevatedButton.icon(
                        icon: Icon(
                            _showStationSelector ? Icons.expand_less : Icons
                                .expand_more),
                          ...
                          children: [
                            ...
                            if (_subdivisions.length > 1)
                              Padding(
                                ...
                                child: DropdownButton<String>(
                                  ...
                                  items: _subdivisions.map<
                                      DropdownMenuItem<String>>((
                                      Map<String, dynamic> subdivision) {
                            ...
                            _isLoadingStations
                              ...
                              itemBuilder: (context, index) {
                                final station = _filteredStations[index];
                                final isSelected = _currentlyPlayingStation !=
                                    null &&
                                    _currentlyPlayingStation!.startsWith(
                                        station['name']);
                                final isFavourite = _favouriteStations.any((
                                    s) =>
                                s['stationuuid'] == station['stationuuid']);

                                return Card(
                                  color: isSelected ? Colors.brown[200] : Colors
                                      .white,
                                  child: ListTile(
              ...
              if (_currentlyPlayingStation != null)
                Row(
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: [
                    if (_currentlyPlayingFavicon != null &&
                        _currentlyPlayingFavicon!.isNotEmpty)
                      ClipRRect(
                    ...
                    if (_currentlyPlayingHomepage != null &&
                        _currentlyPlayingHomepage!.isNotEmpty)
                      Padding(
                        padding: const EdgeInsets.only(left: 16.0),
                        child: IconButton(
                          icon: const Icon(
                              Icons.home, size: 32, color: Colors.brown),
                          tooltip: 'Visit Homepage',
                          onPressed: () =>
                              _launchURL(_currentlyPlayingHomepage!),
                        ),
                      ),
                  ],
                ),
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final String imageUrl = widget.searchTerm != null &&
            ...
            ? 'https://loremflickr.com/400/100/${Uri.encodeComponent(
            widget.searchTerm!)},landscape?lock=$randomId'
            : 'https://loremflickr.com/400/100/landscape?lock=$randomId';

        return GestureDetector(
          onPanUpdate: (details) {
            final newPosition = widget.needlePosition +
                (details.delta.dx / dialWidth);
            widget.onNeedleChanged(newPosition.clamp(0.0, 1.0));
          },
          onPanEnd: (details) {
            if (widget.stationCount > 0) {
              final stationIndex = (widget.needlePosition *
                  (widget.stationCount - 1)).round();
              ...
              children: [
                ...
                AnimatedBuilder(
                  ...
                  builder: (context, child) {
                    final double jitter = widget.isTuning ? (_jitterController
                        .value * 0.01 - 0.005) : 0.0;
                    return TweenAnimationBuilder<double>(
                      tween: Tween<double>(begin: widget.needlePosition,
                          end: widget.needlePosition),
                      duration: const Duration(milliseconds: 300),
                      curve: Curves.easeOut,
                      builder: (context, pos, child) {
                        return Positioned(
                          left: (pos + jitter).clamp(0.0, 1.0) *
                              (dialWidth - 4) - 1,
                          top: 4,
                          bottom: 4,
                          child: child!,
    ...
    return Column(
      children: [
        Text(label, style: const TextStyle(
            fontSize: 10, fontWeight: FontWeight.bold, color: Colors.brown)),
        const SizedBox(height: 4),
        GestureDetector(
          onPanUpdate: (details) {
            ...
            final double angle = atan2(
                localPos.dy - center.dy, localPos.dx - center.dx);
